...
def get_redis_connection():
    for _ in range(10):
        try:
            redis_connection = aioredis.Redis(host='redis')
...
async def cleanup():
    ...
    try:
        ...
        await redis_connection.json().delete('*')
    except redis.exceptions.ConnectionError:
        pass
    return 200
...
async def get_recs(user_id: str):
    with REQUEST_LATENCY.labels(endpoint='/recs').time():
        ...
        try:
            top_items = await redis_connection.json().get('top_items')
            lightfm_items = await redis_connection.json().get(f'lightfm_recommendations:{user_id}')
            unseen_random_items = await redis_connection.json().get(f'unseen_random_items:{user_id}')
        except redis.exceptions.ConnectionError:
        ...
        if random.random() < EPSILON:
            ...
            starting_size = len(item_ids)
            item_ids.extend(np.random.choice(list(unique_item_ids - set(item_ids)), size=TOP_K - starting_size,
